...
var require_utf8 = __commonJS({
	"node_modules/@msgpack/msgpack/dist/utils/utf8.js"(exports) {
		...
		var TEXT_ENCODING_AVAILABLE =
			(typeof process === "undefined" || ((_a = process === null || process === void 0 ? void 0 : process.env) === null || _a === void 0 ? void 0 : _a["TEXT_ENCODING"]) !== "never") &&
	...
	constructor(config) {
		...
		__privateAdd(this, _onMessageFromWorker, async (worker, message) => {
			...
			switch (message.type) {
				...
				case "request":
					throw new WebRChannelError("Can't send messages of type 'request' from a worker. Please Use 'sync-request' instead.");
			}
		});
	...
	constructor(config) {
		...
		__privateAdd(this, _onMessageFromWorker2, async (worker, message) => {
			...
			switch (message.type) {
				...
				case "request":
					throw new WebRChannelError("Can't send messages of type 'request' from a worker.Use service worker fetch request instead.");
			}
		});
		const initWorker = (worker) => {
			...
			__privateMethod(this, _registerServiceWorker, registerServiceWorker_fn)
		...
		if (isCrossOrigin(config.serviceWorkerUrl)) {
			newCrossOriginWorker(`${config.serviceWorkerUrl}webr-worker.js`, (worker) => initWorker(worker));
		} else {
...
function safeEval(call, env) {
	return Module.getWasmTableEntry(Module.GOT.ffi_safe_eval.value)(handlePtr(call), handlePtr(env));
}
...
var _RInteger = class extends RVectorAtomic {
	...
	toTypedArray() {
		return new Int32Array(Module.HEAP32.subarray(Module._INTEGER(this.ptr) / 4, Module._INTEGER(this.ptr) / 4 + this.length));
	}
};
...
var _RDouble = class extends RVectorAtomic {
	...
	toTypedArray() {
		return new Float64Array(Module.HEAPF64.subarray(Module._REAL(this.ptr) / 8, Module._REAL(this.ptr) / 8 + this.length));
	}
};
...
var _RComplex = class extends RVectorAtomic {
	...
	toTypedArray() {
		return new Float64Array(Module.HEAPF64.subarray(Module._COMPLEX(this.ptr) / 8, Module._COMPLEX(this.ptr) / 8 + 2 * this.length));
	}
	toArray() {
		const arr = this.toTypedArray();
		return this.detectMissing().map((m, idx) => (m ? null : { re: arr[2 * idx], im: arr[2 * idx + 1] }));
	}
};
...
var _RCharacter = class extends RVectorAtomic {
	...
	toTypedArray() {
		return new Uint32Array(Module.HEAPU32.subarray(Module._STRING_PTR(this.ptr) / 4, Module._STRING_PTR(this.ptr) / 4 + this.length));
	}
	toArray() {
		return this.detectMissing().map((m, idx) => (m ? null : Module.UTF8ToString(Module._R_CHAR(Module._STRING_ELT(this.ptr, idx)))));
	}
};
...
var _RRaw = class extends RVectorAtomic {
	...
	toTypedArray() {
		return new Uint8Array(Module.HEAPU8.subarray(Module._RAW(this.ptr), Module._RAW(this.ptr) + this.length));
	}
};
var RRaw = _RRaw;
...
function newRObject(data, objType) {
	...
	const obj = new RClass(replaceInObject(data, isWebRPayloadPtr, (t) => RObject.wrap(t.obj.ptr)));
	return {
		obj: {
			type: obj.type(),
			ptr: obj.ptr,
...
function captureR(code, options = {}) {
	...
	try {
		...
		if (_options.captureConditions && _options.throwJsException) {
			...
			if (error) {
				throw new Error(((_a = error.pluck("data", "message")) == null ? void 0 : _a.toString()) || "An error occured evaluating R code.");
			}
		}
		return capture;
	} finally {
